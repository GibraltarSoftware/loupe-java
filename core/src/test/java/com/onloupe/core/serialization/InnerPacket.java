...
public class InnerPacket implements IPacket {
	...
	public InnerPacket(String caption) {
		...
		this._SubPacket = new SubPacket(caption, (int) this._ThreadId); // use same caption for both by default
	}

	public InnerPacket(String innerCaption, String subCaption) {
		this._TimeStamp = LocalDateTime.now(); // we convert to UTC during serialization, we want local time.
		this._ThreadId = Thread.currentThread().getId();
		this._Caption = innerCaption; // set InnerPacket caption and...
		this._SubPacket = new SubPacket(subCaption, (int) this._ThreadId); // ...SubPacket caption separately
	}
	...
	public void writePacketDefinition(PacketDefinition definition) {
		...
		definition.getFields().add("innerCaption", FieldType.STRING);
	...
	public final void writeFields(PacketDefinition definition, SerializedPacket packet) {
		...
		packet.setField("innerCaption", this._Caption);
	...
	public final void readFields(PacketDefinition definition, SerializedPacket packet) {
		...
		case 1:
			...
			this._Caption = packet.getField("innerCaption", String.class);
	...
	public boolean equals(Object obj) {
		return equals(obj instanceof InnerPacket ? (InnerPacket) obj : null);
	}

	public final boolean equals(InnerPacket other) {
		// Careful - can be null
		if (other == null) {
